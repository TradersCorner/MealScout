...
export function registerStaffRoutes(app: Express) {
   ...
   */
  app.get("/api/admin/staff", isAuthenticated, isAdmin, async (req: any, res) => {
    try {
   ...
   */
  app.post("/api/admin/staff/:userId/promote", isAuthenticated, isAdmin, async (req: any, res) => {
    try {
   ...
   */
  app.post("/api/admin/staff/:userId/demote", isAuthenticated, isAdmin, async (req: any, res) => {
    try {
      const { userId } = req.params;
   ...
   */
  app.post("/api/staff/users", isAuthenticated, isStaffOrAdmin, async (req: any, res) => {
    try {
      ...
      res.json({
        ...
        tempPassword,
        mustResetPassword: true,
        message: "Customer account created. User must reset password on first login.",
      });
    } catch (error) {
   ...
   * Returns temp password that must be reset on first login
   */
  app.post("/api/staff/restaurant-owners", isAuthenticated, isStaffOrAdmin, async (req: any, res) => {
    try {
      const { email, firstName, lastName, phone, restaurantName, restaurantAddress, restaurantPhone } = req.body;
      const staffUser = req.user;
      ...
      res.status(500).json({ error: "Failed to create restaurant owner account" });
    }
  });
   ...
   */
  app.post("/api/account/force-password-reset", isAuthenticated, async (req: any, res) => {
    try {
      const user = req.user;
