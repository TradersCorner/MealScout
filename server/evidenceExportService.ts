 ...
 */
export async function fetchEvidenceSnapshot(videoId: string): Promise<EvidenceSnapshot | null> {
  try {
    ...
    const videoMetadata: VideoMetadata = {
      videoId: video.id,
      type: 'user', // TODO: Determine from schema when restaurant ads are added
    ...
    const decision: ModerationDecision | null = reportResults.length > 0
      ? {
          action: reportResults[0].status === 'action_taken' ? 'hide' : 
                  reportResults[0].status === 'dismissed' ? 'restore' : 'pending' as any,
          reason: reportResults[0].reason || undefined,
          timestamp: reportResults[0].reviewedAt || reportResults[0].createdAt || new Date(),
    ...
    const evidence: EvidenceData | null = allReports.length > 0 ? {
      reportCount: allReports.length,
      reportTimestamps: allReports.map((r: any) => r.createdAt || new Date()),
      reporterIdsRedacted: allReports.map((r: any) => 
        r.reportedByUserId ? r.reportedByUserId.substring(0, 4) + '...' : 'unknown'
      ),
      priorActions: [], // TODO: Add when action history is tracked separately
    } : null;

    // Fetch appeals
    // TODO: Wire to appeals table when available
 ...
 */
export function generateEvidencePDF(snapshot: EvidenceSnapshot): { pdfStream: PDFKit.PDFDocument; contentHash: string } {
  ...
  // Header
  doc.fontSize(20).font('Helvetica-Bold').text('MODERATION EVIDENCE SNAPSHOT', { align: 'center' });
  doc.moveDown();
  doc.fontSize(10).font('Helvetica').fillColor('#666666').text(`Generated: ${snapshot.generatedAt.toISOString()}`, { align: 'center' });
  ...
  contentParts.push(`CREATED:${snapshot.videoMetadata.createdAt.toISOString()}`);
  ...
  if (snapshot.decision) {
    ...
    doc.fillColor('#999999').text('[No moderation decision recorded]').fillColor('#000000');
  }
  doc.moveDown(2);
  ...
  if (snapshot.evidence) {
    ...
    if (snapshot.evidence.priorActions.length > 0) {
      ...
      snapshot.evidence.priorActions.forEach((action) => {
        doc.text(`  - ${action.action.toUpperCase()} at ${action.timestamp.toISOString()}`, { indent: 20 });
        if (action.reason) {
    ...
    doc.fillColor('#999999').text('[No evidence data available]').fillColor('#000000');
  }
  doc.moveDown(2);
  ...
  doc.text(`Video Age: ${Math.floor((snapshot.generatedAt.getTime() - snapshot.videoMetadata.createdAt.getTime()) / (1000 * 60 * 60 * 24))} days`);
  ...
  if (snapshot.appeals.length > 0) {
    snapshot.appeals.forEach((appeal) => {
      ...
      doc.text(`  Submitted: ${appeal.submittedAt.toISOString()}`, { indent: 20 });
  ...
  // Immutability Notice
  doc.fontSize(12).font('Helvetica-Bold').fillColor('#CC0000').text('IMMUTABILITY NOTICE', { align: 'center' });
  ...
  // Footer with hash
  doc.moveDown(2);
  doc.fontSize(8).font('Helvetica').fillColor('#999999').text(`Content Hash (SHA-256): ${contentHash}`, { align: 'center' });
  doc.text(`Video ID: ${snapshot.videoMetadata.videoId} | Generated: ${snapshot.generatedAt.toISOString()}`, { align: 'center' });
