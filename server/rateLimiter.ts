 ...
 */
export const rateLimitByUser = (limit: number, window?: number) => {
  return rateLimit(limit, window, (req) => {
 ...
 */
export const rateLimitByIp = (limit: number, window?: number) => {
  return rateLimit(limit, window, (req) => `ip:${req.ip}`);
};
