 ...
 */
export function LocationDebug(_a) {
    var state = _a.state, lastConfirmedAt = _a.lastConfirmedAt, restaurantId = _a.restaurantId;
    if (import.meta.env.PROD) {
        return <LocationDot state={state} lastConfirmedAt={lastConfirmedAt}/>;
    }
    var timeAgo = lastConfirmedAt
        ? "".concat(Math.round((Date.now() - new Date(lastConfirmedAt).getTime()) / 60000), "m ago")
        : "never";
