...
async function getReverseGeocodedLocationName(
  ...
  try {
    // Avoid Nominatim CORS issues in production; only call in development
    if (
      typeof window !== "undefined" &&
      window.location.hostname === "www.mealscout.us"
    ) {
      console.log(
        "🌍 Skipping reverse geocoding in production; using coordinates instead"
      );
      return;
    }

    // Try reverse geocoding with OpenStreetMap Nominatim (development / staging only)
