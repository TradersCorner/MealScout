...
const ESCALATION_MINUTES = {
  initial: 0,
  noAcknowledgment: parseInt(
    process.env.INCIDENT_ESCALATION_NO_ACK_MINUTES || "15"
  ),
  noResolution: parseInt(
    process.env.INCIDENT_ESCALATION_NO_RESOLUTION_MINUTES || "120"
  ),
  possibleBreach: parseInt(
    process.env.INCIDENT_ESCALATION_BREACH_MINUTES || "1440"
  ), // 24 hours
  hardEscalation: parseInt(
    process.env.INCIDENT_ESCALATION_HARD_MINUTES || "4320"
  ), // 72 hours
};
...
export async function getCurrentOnCall(): Promise<{
  userId: string;
  isPrimary: boolean;
} | null> {
  const now = new Date();
  const rotations = await db
    ...
    .from(oncallRotation)
    .where(
      and(lte(oncallRotation.startDate, now), gte(oncallRotation.endDate, now))
    )
...
async function notifyIncident(
  incident: any,
  eventType: "created" | "acknowledged" | "escalated"
) {
  const rule = Object.values(ANOMALY_RULES).find(
    (r) => r.id === incident.ruleId
  );
  const message = `[${incident.severity.toUpperCase()}] ${
    rule?.name || "Incident"
  } - ${eventType}`;
  ...
  if (
    ...
    NOTIFICATION_CONFIG.email.recipients.length > 0
  ) {
    const subject = `[${incident.severity.toUpperCase()}] ${
      rule?.name || "Incident"
    } - ${eventType}`;
    const html = `
          ...
          attachments: [
            {
              ...
              fields: [
                ...
                {
                  title: "Rule",
                  value: rule?.name || incident.ruleId,
                  short: false,
                },
              ],
            },
...
- **Acknowledged:** ${incident.acknowledgedAt || "N/A"} (by: ${
    incident.acknowledgedBy || "N/A"
  })
- **Resolved:** ${incident.resolvedAt || "N/A"} (by: ${
    incident.resolvedBy || "N/A"
  })
...
`;
  ...
  // Persist report to disk under audit_reports
  try {
    ...
    auditLogger.info("Incident report saved", { filePath });
  } catch (err) {
    auditLogger.error("Failed to save incident report", { error: String(err) });
  }
...
export async function checkEscalations() {
  ...
  for (const incident of openIncidents) {
    ...
    const minutesOpen =
      (now.getTime() - incident.createdAt.getTime()) / (1000 * 60);
  ...
  for (const incident of acknowledgedIncidents) {
    const minutesSinceAck =
      (now.getTime() - new Date(incident.acknowledgedAt!).getTime()) /
      (1000 * 60);
