...
export default function DealCard({ deal }: DealCardProps) {
  ...
  const isGoldenForkUser = Boolean((user as any)?.influenceScore && (user as any)?.influenceScore > 0);
  ...
  useEffect(() => {
    ...
    const fetchFavoritesSnapshot = async () => {
      try {
        ...
        const isFav = list.some((fav: any) => (fav.restaurantId || fav.restaurant?.id) === deal.restaurantId);
  ...
  const toggleRestaurantFavorite = async (nextSelected: boolean) => {
    ...
    if (nextSelected && !isRestaurantFavorite && favoriteCount !== null && favoriteCount >= MAX_FAVORITES) {
    ...
    try {
      ...
      if (nextSelected) {
        await apiRequest('POST', `/api/restaurants/${deal.restaurantId}/favorite`, {});
        setIsRestaurantFavorite(true);
        setFavoriteSelection(true);
        setFavoriteCount((prev) => (prev ?? 0) + (isRestaurantFavorite ? 0 : 1));
      } else {
        await apiRequest('DELETE', `/api/restaurants/${deal.restaurantId}/favorite`, {});
  ...
  const handleRecommendSubmit = async () => {
    ...
    if (favoriteSelection && !isRestaurantFavorite && favoriteCount !== null && favoriteCount >= MAX_FAVORITES) {
      setFavoriteError(`You can favorite up to ${MAX_FAVORITES} restaurants.`);
      return;
    }
    ...
    <div>
        ...
        <CardContent className="p-0">
          ...
          <div className="relative h-24 bg-gradient-to-br from-gray-50 to-gray-100 overflow-hidden rounded-t-2xl">
            ...
            <div className="absolute top-1.5 left-1.5 bg-orange-600 text-white px-1.5 py-0.5 rounded-lg shadow-lg">
              <span className="font-bold text-sm leading-none">{formatDiscount()} OFF</span>
            </div>
            ...
            >
              <GoldenForkIcon className={`w-3.5 h-3.5 transition-colors duration-200 ${forkPressed ? 'text-amber-500' : 'text-gray-700'}`} />
            </button>
            
            {/* Restaurant Name Overlay - bottom */}
            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-1.5">
              <h3 className="font-bold text-white text-xs truncate" data-testid={`text-restaurant-name-${deal.id}`}>
                {deal.restaurant?.name || 'Restaurant Name'}
              </h3>
              {deal.restaurant?.cuisineType && (
                <p className="text-white/90 text-[10px] truncate">{deal.restaurant.cuisineType}</p>
              )}
            </div>
          </div>
          ...
          <div className="p-2" onClick={handleCardClick}>
            {/* Deal Title */}
            <p className="text-gray-900 text-xs font-bold mb-1.5 line-clamp-2 leading-tight min-h-[2rem]" data-testid={`text-restaurant-info-${deal.id}`}>
              {deal.title}
            </p>
            ...
            <div className="flex items-center gap-1.5 mb-1.5 text-[10px] text-gray-600">
              ...
              {deal.minOrderAmount && (
                <>
                  <span>•</span>
                  <span className="text-orange-600 font-medium">${deal.minOrderAmount} min</span>
                </>
              )}
            </div>
            ...
            <div className="flex items-center gap-2 text-[10px] text-gray-500 mb-2">
              ...
              <div className="flex items-center gap-0.5">
                ...
                <span className="font-medium text-gray-700">{deal.currentUses || 188} claimed</span>
              </div>
            </div>
      ...
      {showRecommendModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          <div className="absolute inset-0 bg-black/50" onClick={() => setShowRecommendModal(false)} />
          <div className="relative w-full max-w-md rounded-3xl shadow-2xl overflow-hidden border border-yellow-200/60">
            ...
            <div className="bg-gradient-to-br from-yellow-200 via-amber-200 to-yellow-300 px-5 py-4 flex items-start gap-3">
              ...
              <div className="flex-1">
                <h3 className="text-lg font-bold text-yellow-900 leading-tight">Recommend this restaurant</h3>
                <p className="text-xs text-yellow-800/80">Your recommendation directly affects local visibility</p>
                {isGoldenForkUser && (
                  <div className="mt-1 text-xs text-yellow-900 font-semibold flex items-center gap-1">
                    <span role="img" aria-label="golden">🥇</span>
            ...
            <div className="bg-white px-5 pb-5 pt-4">
              <label className="block text-sm font-semibold text-gray-900 mb-1">Add context (optional)</label>
              <p className="text-xs text-gray-600 mb-2">What makes this spot worth recommending?</p>
              <textarea
              ...
              <button
                ...
                onClick={() => {
                  ...
                  if (next && favoriteCount !== null && favoriteCount >= MAX_FAVORITES && !isRestaurantFavorite) {
                    setFavoriteError(`You can favorite up to ${MAX_FAVORITES} restaurants.`);
                  } else {
                    setFavoriteError("");
                  }
              ...
              >
                <div className={`w-9 h-9 rounded-lg flex items-center justify-center border ${favoriteSelection ? 'bg-white text-yellow-600 border-yellow-300' : 'bg-white text-gray-500 border-gray-200'}`}>
                  <Star className={`w-5 h-5 ${favoriteSelection ? 'fill-yellow-500 text-yellow-600' : ''}`} />
                </div>
                <div className="flex-1">
                  <div className="font-semibold text-gray-900 text-sm">Add to Favorites</div>
                  <div className="text-xs text-gray-600">Only 3 favorites allowed{favoriteSelection ? ' · One of your top 3 restaurants' : ''}</div>
                  {favoriteCount !== null && (
                    <div className="text-[11px] text-gray-500 mt-0.5">Currently using {favoriteCount}/{MAX_FAVORITES}</div>
                  )}
                  {favoriteError && (
                    <div className="text-red-600 text-xs mt-1">{favoriteError}</div>
                  )}
                </div>
              </button>
