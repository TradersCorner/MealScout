...
export function SEOFAQ({ items, title = "Frequently Asked Questions", defaultExpanded = false, className = "" }: SEOFAQProps) {
  ...
  return (
    <section className={`seo-faq ${className}`} itemScope itemType="https://schema.org/FAQPage">
      <h2 className="text-xl font-bold mb-4 text-[color:var(--text-primary)]">{title}</h2>
      <div className="space-y-3">
        {items.map((item, index) => (
          ...
          >
            ...
            >
              <h3 itemProp="name" className="font-semibold text-[color:var(--text-primary)] pr-4">
                {item.question}
              </h3>
            ...
            >
              <div itemProp="text" className="text-[color:var(--text-secondary)] leading-relaxed">
                {item.answer}
              </div>
            </div>
          </div>
...
export function MinimalFAQ({ items, title = "Common Questions", className = "" }: SEOFAQProps) {
  return (
    <section className={`minimal-faq text-sm text-[color:var(--text-muted)] ${className}`} itemScope itemType="https://schema.org/FAQPage">
      <h3 className="font-medium text-[color:var(--text-secondary)] mb-3">{title}</h3>
      <div className="space-y-4">
        {items.map((item, index) => (
          <div key={index} itemScope itemProp="mainEntity" itemType="https://schema.org/Question">
            <h4 itemProp="name" className="font-medium text-[color:var(--text-primary)] mb-1">
              {item.question}
            </h4>
            <div itemScope itemProp="acceptedAnswer" itemType="https://schema.org/Answer">
              <p itemProp="text" className="text-[color:var(--text-muted)] leading-relaxed">
                {item.answer}
              </p>
            </div>
          </div>
