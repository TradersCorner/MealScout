...
export default function RoleLandingPage(_a) {
    var content = _a.content, mapSlot = _a.mapSlot, _b = _a.theme, theme = _b === void 0 ? "default" : _b;
    var canonicalUrl = typeof window !== "undefined"
        ? "".concat(window.location.origin).concat(content.seo.canonicalPath)
        : undefined;
    var themeStyles = theme === "ember"
        ? {
            "--bg-base": "#1C1A18",
            ...
            "--map-bg": "linear-gradient(140deg, rgba(63,50,40,0.2), rgba(80,60,42,0.35) 50%, rgba(42,36,30,0.25))",
        }
        : {
            "--bg-base": "#fff8ee",
            ...
            "--map-bg": "linear-gradient(140deg, #f6efe1, #fff4e6 50%, #e6eeff)",
        };
    return (<div className="min-h-screen px-4 py-12" style={__assign({ background: "var(--bg-glow), var(--bg-glow-2), var(--bg-base)", fontFamily: '"Space Grotesk", "IBM Plex Sans", "Segoe UI", sans-serif' }, themeStyles)}>
      <SEOHead title={content.seo.title} description={content.seo.description} keywords={content.seo.keywords} canonicalUrl={canonicalUrl} noIndex={content.seo.noIndex}/>
      ...
      <div className="max-w-6xl mx-auto space-y-12">
        <section className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
          <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <Button asChild className="px-6 py-6 text-base text-[var(--cta-text)] bg-[linear-gradient(135deg,var(--accent),var(--accent-strong))] hover:opacity-90 active:translate-y-[1px]">
                <Link href={content.primaryCta.href}>
                  {content.primaryCta.label}
                </Link>
              </Button>
              {content.secondaryCta && (<Button asChild variant="outline" className="px-6 py-6 text-base border-[var(--border-soft)] text-[var(--ink)] hover:bg-transparent active:translate-y-[1px]">
            ...
            <div className="flex flex-wrap gap-3 text-xs text-[var(--ink-soft)]">
              {content.bullets.map(function (bullet) { return (<span key={bullet} className="inline-flex items-center gap-1">
            ...
            <Card className="border shadow-2xl" style={{ backgroundColor: "var(--card)", borderColor: "var(--border)" }}>
              <CardContent className="p-6 space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  {content.map.badge && (<div className="flex items-center gap-1 rounded-full px-3 py-1 text-xs font-semibold" style={{ backgroundColor: "var(--card-muted)", color: "var(--ink-dark-muted)" }}>
                      {content.map.badge}
                    </div>)}
                </div>
                <div className="relative h-48 rounded-2xl overflow-hidden border" style={{ borderColor: "var(--border)", background: "var(--map-bg)" }}>
                  {mapSlot !== null && mapSlot !== void 0 ? mapSlot : (<>
                      ...
                      {content.map.hint && (<div className="absolute left-6 bottom-6 rounded-full px-3 py-1 text-xs font-semibold shadow" style={{ backgroundColor: "var(--card)", color: "var(--ink-dark)" }}>
                          {content.map.hint}
                        </div>)}
                    </>)}
                </div>
                <div className="grid grid-cols-2 gap-3">
                  {content.stats.map(function (stat) { return (<div key={stat.label} className="rounded-xl border px-4 py-3" style={{ backgroundColor: "var(--card)", borderColor: "var(--border)" }}>
        ...
        <section className="grid gap-4 rounded-3xl border p-6 text-sm shadow-lg md:grid-cols-3" style={{ backgroundColor: "var(--card-muted)", borderColor: "var(--border)", color: "var(--ink-dark-muted)" }}>
          {content.valueProps.map(function (value) { return (<div key={value.text} className="flex items-center gap-3">
        ...
        <section className="grid gap-6 md:grid-cols-3">
          {content.steps.map(function (step) { return (<Card key={step.title} className="border" style={{ backgroundColor: "var(--card)", borderColor: "var(--border)" }}>
          ...
          <Card className="border" style={{ backgroundColor: "var(--card)", borderColor: "var(--border)" }}>
            <CardContent className="p-6 space-y-4">
              ...
              <div className="grid gap-3 text-sm text-[var(--ink-dark-muted)]">
                {content.reasons.items.map(function (item) { return (<div key={item} className="flex items-start gap-3">
          ...
          <Card className="border shadow-xl" style={{ backgroundColor: "var(--panel-dark-bg)", borderColor: "var(--panel-dark-border)" }}>
            <CardContent className="p-6 space-y-3">
              ...
              <ul className="text-sm text-[var(--ink)] space-y-2">
                {content.starter.bullets.map(function (bullet) { return (<li key={bullet} className="flex items-center gap-2">
        ...
        <section className="rounded-3xl px-6 py-10 text-center shadow-2xl" style={{ backgroundColor: "var(--panel-dark-bg)" }}>
          ...
          <div className="mt-6 flex flex-col sm:flex-row justify-center gap-3">
            ...
            {content.finalCta.secondary && (<Button asChild variant="outline" className="px-6 py-6 text-base text-[var(--cta-muted-text)] border-[var(--border-soft)] hover:text-[var(--cta-muted-text)] active:translate-y-[1px]">
