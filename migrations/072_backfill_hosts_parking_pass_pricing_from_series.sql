...
BEGIN
  IF EXISTS (
    ...
    SET
      ...
      parking_pass_start_time = COALESCE(s.default_start_time, parking_pass_start_time),
      parking_pass_end_time = COALESCE(s.default_end_time, parking_pass_end_time),
      parking_pass_days_of_week = COALESCE(s.parking_pass_days_of_week, parking_pass_days_of_week),
      updated_at = NOW()
    FROM event_series s
