...
import { Loader2, Plus, Calendar, Clock, Truck, Users, ChefHat, AlertCircle } from "lucide-react";
...
function HostDashboard() {
  ...
  const handleUpdateStatus = async (interestId: number, status: 'accepted' | 'declined') => {
    try {
      ...
      // Update local state
      setEventInterests(prev => prev.map(i => 
  ...
  const handleSeriesCreated = async () => {
    toast({
      ...
      description: "Your event series has been published and is now discoverable by trucks.",
    });
  ...
  const handleEventUpdated = async () => {
    toast({
      title: "Event Updated",
      description: "Changes have been saved. Other occurrences are not affected.",
    });
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">{host.businessName}</h1>
          <p className="text-slate-600">{host.address}</p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => setIsCreatingSeriesDialog(true)}>
            <Calendar className="mr-2 h-4 w-4" /> Open Call (Series)
          </Button>
          <Button onClick={() => setIsCreating(!isCreating)}>
            {isCreating ? "Cancel" : <><Plus className="mr-2 h-4 w-4" /> Single Event</>}
          </Button>
        </div>
      </div>
            ...
            <div className="flex items-center space-x-4 border p-4 rounded-md border-slate-200 bg-slate-50">
              ...
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <Label htmlFor="hard-cap" className="font-semibold">Capacity Guard v2.2</Label>
                  <Badge variant="secondary" className="text-xs">New</Badge>
                </div>
                <p className="text-sm text-slate-500">
                  Strictly enforces the max trucks limit. Once you accept enough trucks to hit the limit, no further approvals will be allowed.
                </p>
              </div>
            </div>
                    ...
                    {series
                      ...
                      .map((eventSeries) => {
                        ...
                        const futureOccurrences = events.filter(
                          e => e.seriesId === String(eventSeries.id) && new Date(e.date) >= new Date(new Date().setHours(0,0,0,0))
                        );
                
                        const affectedTruckIds = new Set(
                          futureOccurrences.flatMap(e => e.interests?.map(i => i.truckId) || [])
                        );

                        return (
                          <div key={eventSeries.id} className="border border-slate-200 rounded-lg p-4 bg-slate-50">
                            <div className="flex items-start justify-between">
                              <div className="flex-1">
                                <h3 className="font-semibold text-slate-900">{eventSeries.name}</h3>
                                <div className="flex items-center gap-4 mt-2 text-sm text-slate-600">
                                  <span className="flex items-center gap-1">
                                    <Calendar className="h-4 w-4" />
                                    {futureOccurrences.length} future occurrence{futureOccurrences.length !== 1 ? 's' : ''}
                                  </span>
                                  <span className="flex items-center gap-1">
                                    <Truck className="h-4 w-4" />
                                    {affectedTruckIds.size} interested truck{affectedTruckIds.size !== 1 ? 's' : ''}
                                  </span>
                                  <span className="flex items-center gap-1">
                                ...
                                <p className="text-xs text-slate-500 mt-2">
                                  {eventSeries.recurrenceRule} • Timezone: {eventSeries.timezone}
                                </p>
                              </div>
                              <Button
                                variant="destructive"
          ...
          <TabsContent value="upcoming" className="space-y-4">
            {events.filter(e => new Date(e.date) >= new Date(new Date().setHours(0,0,0,0))).length === 0 ? (
              <div className="text-center py-12 bg-slate-50 rounded-xl border border-dashed border-slate-300">
                ...
                <p className="text-slate-500 mb-4">Create an event to invite food trucks to your location.</p>
                <Button variant="outline" onClick={() => setIsCreating(true)}>Schedule First Event</Button>
              </div>
            ) : (
              <div className="grid gap-4">
                {events
                  ...
                  .map((event) => (
                  <div key={event.id} className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between">
                    <div className="flex items-center gap-6">
                      ...
                      <div>
                        <div className="flex items-center gap-4 text-sm text-slate-600 mb-1">
                          ...
                          <span className="flex items-center gap-1">
                            <Truck className="h-4 w-4" />
                            {event.maxTrucks} Truck{event.maxTrucks !== 1 ? 's' : ''}
                          </span>
                          {event.hardCapEnabled && (
                            <span title="Capacity Guard v2.2 Enabled" className="flex items-center gap-1 text-xs bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-100">
                              <AlertCircle className="h-3 w-3 text-emerald-600" />
                              <span className="text-emerald-700 font-medium">Strict Cap</span>
                            </span>
                          )}
                        </div>
                        <div className="flex items-center gap-2 mt-2">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                            ...
                            event.status === 'filled' ? 'bg-blue-100 text-blue-800' : 
          ...
          <TabsContent value="past" className="space-y-4">
            {events.filter(e => new Date(e.date) < new Date(new Date().setHours(0,0,0,0))).length === 0 ? (
              <div className="text-center py-12 bg-slate-50 rounded-xl border border-dashed border-slate-300">
                ...
                <h3 className="text-lg font-medium text-slate-900">No past events</h3>
                <p className="text-slate-500">Your event history will appear here.</p>
              </div>
            ) : (
              <div className="grid gap-4 opacity-75">
                {events
                  .filter(e => new Date(e.date) < new Date(new Date().setHours(0,0,0,0)))
                  .map((event) => (
                  <div key={event.id} className="bg-slate-50 p-6 rounded-xl border border-slate-200 flex items-center justify-between">
                    <div className="flex items-center gap-6">
            ...
            <SheetDescription>
              {selectedEvent && (
                <div className="space-y-4">
                  <span>
                    For event on {format(new Date(selectedEvent.date), 'MMMM d, yyyy')}
                  </span>
                  ...
                  <div className="space-y-2 mt-4">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className={`font-medium ${
                        eventInterests.filter(i => i.status === 'accepted').length >= selectedEvent.maxTrucks 
                          ? 'text-amber-600' 
                          : 'text-slate-900'
                      }`}>
                        {eventInterests.filter(i => i.status === 'accepted').length} / {selectedEvent.maxTrucks} Spots Filled
                      </span>
                    </div>
                    <Progress 
                      value={(eventInterests.filter(i => i.status === 'accepted').length / selectedEvent.maxTrucks) * 100} 
                      className={`h-2 ${
                        eventInterests.filter(i => i.status === 'accepted').length >= selectedEvent.maxTrucks 
                    ...
                    {eventInterests.filter(i => i.status === 'accepted').length >= selectedEvent.maxTrucks && (
                      <Alert variant="default" className="bg-amber-50 border-amber-200 text-amber-800 mt-2">
                        <AlertCircle className="h-4 w-4 text-amber-600" />
                        <AlertTitle className="text-amber-800">Capacity Reached</AlertTitle>
                        <AlertDescription className="text-amber-700">
                          You have reached your maximum capacity of {selectedEvent.maxTrucks} trucks. You can still accept more, but be aware of space limitations. This does not reserve space or guarantee attendance.
                        </AlertDescription>
                      </Alert>
                    )}
                  </div>
              ...
              <ScrollArea className="h-[calc(100vh-200px)] pr-4">
                <div className="space-y-4">
                  {eventInterests.map((interest) => (
                    <div key={interest.id} className="flex items-start gap-4 p-4 rounded-lg border border-slate-200 bg-slate-50/50">
                      <Avatar className="h-12 w-12 border border-slate-200">
                        <AvatarImage src={interest.truck.imageUrl} />
                        <AvatarFallback><ChefHat className="h-6 w-6 text-slate-400" /></AvatarFallback>
                      </Avatar>
                      
                      <div className="flex-1">
                        <div className="flex items-start justify-between">
                          <div>
                            <h4 className="font-semibold text-slate-900">{interest.truck.name}</h4>
                            <p className="text-sm text-slate-500">{interest.truck.cuisineType}</p>
                          </div>
                          ...
                          {interest.status === 'pending' ? (
                            <>
                              ...
                              <Button 
                                ...
                                className="w-full" 
                                variant="outline"
                                onClick={() => handleUpdateStatus(interest.id, 'declined')}
                              >
                                Decline
                              </Button>
                            </>
                          ) : (
                            <div className={`w-full text-center py-2 text-sm font-medium rounded-md ${
